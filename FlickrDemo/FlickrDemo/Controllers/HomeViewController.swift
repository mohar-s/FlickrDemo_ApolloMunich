...
// it will show listing of images , using Flickr API
...
class HomeViewController: UIViewController, UICollectionViewDelegate , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout, UISearchBarDelegate {
    ...
    var searchText : String = "nature"   // Default search string
    var perpage : Int = 20  // limit to fetch photo in 1 request.
    var pages : Int = 0   // it will store total page , we will get in response
    var currentpageIndex : Int = 1
    
    var isLoading : Bool = true  // check service status
    
    
    let photosArr  : NSMutableArray = NSMutableArray()  // store Photos in array
